...
(function() {
	...
	document.write([
		...
		'<div class="demo-header">',
			...
			info.title ? [
				'<h1>',
					'<span class="demo-title">' + getStripTags(info.title) + '</span>',
					info.viewsource ? '<a href="#" class="demo-header-viewsrc demo-button" id="' + getStripTags(viewsrcId) + '" title="소스보기" style="right:' + (info.qrcode ? 32 : 0) + 'px;"><span class="demo-span">소스보기</span></a>' : '',
					info.viewsource ? (
						'<div class="demo-viewsrc-layer demo-hide" id="' + getStripTags(viewsrcLayerId) + '"><div class="demo-content">' +
							'<code data-selector="*"></code>' +
						'</div></div>'
					) : '',
					info.qrcode ? '<a href="#" class="demo-header-qrcode demo-button" id="' + getStripTags(qrcodeId) + '" title="QR 코드"><span class="demo-span">QR 코드</span></a>' : '',
					info.qrcode ? '<div id="' + getStripTags(qrcodeLayerId) + '" class="demo-qrcode-layer demo-hide"><img src="http://chart.apis.google.com/chart?cht=qr&chs=220x220&chl=' + encodeURIComponent(location.href) + '&choe=UTF-8" /></div>' : '',
				'</h1>'
			].join('\n') : '',
			info.subtitle ? '<h2><span class="demo-span demo-subtitle">' + getStripTags(info.subtitle) + '</span></h2>' : '',
			info.description ? '<p class="demo-desc">' + getStripTags(info.description) + '</p>' : '',
		'</div>',
		'<div class="demo-body">'
	].join('\n'));
